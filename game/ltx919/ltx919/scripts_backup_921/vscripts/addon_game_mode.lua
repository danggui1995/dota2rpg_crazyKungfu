...
--GameRules.xg_num = 0
GameRules.task_num = {npc_task_num_ji_normal=0,npc_task_num_xiongge=0}
--GameRules.ji_num = 0
...
GameRules.chuguai_t = 
{
	...
	3000,3150,3300,3450,3600,3750,3900,4050,4200,
	4350,4500
}
...
function ForgeItem( event,data )
	...
	local __forge_table = GameRules.ForgeTable
	...
	for k,v in pairs(__forge_table) do
		...
		if bHasForged then
			...
			if rd<forge_chance then
				...
				Notifications:MidLeft({text="forge_result_succeed", duration=3.0,style={color="red",["font-size"]="40px"}})
			else
				Notifications:MidLeft({text="forge_result_failed", duration=3.0,style={color="red",["font-size"]="40px"}})
			end
			return 
		end
	end
	Notifications:MidLeft({text="no_forge_result", duration=3.0,style={color="red",["font-size"]="40px"}})
end
...
function OnBaseUpArmor( index,keys )
	...
	if opt == 1 then
		...
		if nGold <2000 then
			...
			StrengthBase(1000,10)
			local hero = PlayerResource:GetSelectedHeroEntity(id)
			hero:SpendGold(2000,DOTA_ModifyGold_PurchaseItem)
		end
	else
		local id = keys.PlayerID
		if GameRules.hero_shengwang[id+1] <5 then
			...
			StrengthBase(1000,10)
		   	GameRules.hero_shengwang[id+1] = GameRules.hero_shengwang[id+1] - 5
		end
	end
...
function StartSLZL( )
	local timeNow = math.floor(GameRules:GetGameTime())
	if GameRules.gw_lv == 10 and timeNow == GameRules.chuguai_t[GameRules.gw_lv]-GameRules.slzl[5][1] then
		Timers:CreateTimer({
...
function RollDrops(unit,attacker)
    ...
    if DropInfo then
        for k,ItemTable in pairs(DropInfo) do
            ...
            ItemTable.Chance = (ItemTable.Chance or 0) + math.floor(math.sqrt(math.floor(math.sqrt(GameRules.hero_shengwang[attacker:GetPlayerOwnerID()+1]))))
...
function CAddonTemplateGameMode:ResetRebornTime( keys )
	...
	if string.find(__killedName,"fb")~=nil then
	...
	if string.find(__killedName,"myboss")~=nil then
		...
		GameRules:GetGameModeEntity():SetContextThink(DoUniqueString("reborn_dxg"),
			function()
