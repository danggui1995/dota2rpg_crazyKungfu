...
function ManaBreak( keys )
	...
	local damageXs = ability:GetLevelSpecialValueFor("damage_xs", (ability:GetLevel() - 1))
	local diff = target:GetMana() - manaBurn
	if diff>=0 then
		target:SetMana(diff)
	else
		target:SetMana(1)
	end
	local totalDamage = (target:GetMaxMana()-target:GetMana())*damageXs
	local damageTable = {}
...
function OnBaseAttacked( data )
	...
	if ability:IsCooldownReady() then
		if hCaster:GetHealthPercent()<50 then
			...
			EmitGlobalSound("announcer_dlc_bristleback_bris_ann_ancient_attack_follow_up_03")
		else
			Notifications:TopToAll({text="#zaoshougongji",duration=2,style={color="red"},continue=false})
			EmitGlobalSound("announcer_dlc_bristleback_bris_ann_rax_attack_follow_up_02")
		end
