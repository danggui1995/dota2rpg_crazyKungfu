...
function BehaviorBack:Evaluate()
	...
	if GameRules.slzl[6][1]==0 and (thisEntity:GetAbsOrigin() - Spawn_pos):Length2D()>700 then
		desire = 11
	end
...
function BehaviorAttackMove:Evaluate()
	...
	if not targetHero and GameRules.slzl[6][1]==1 then
		desire = 4
	end
	return desire
end
...
function BehaviorAttackTransform:Evaluate()
	local targetHero = AICore:GetNearstUnit(thisEntity,800)
	local desire = 0
	if targetHero~=nil and GameRules.slzl[6][1]==1 then
