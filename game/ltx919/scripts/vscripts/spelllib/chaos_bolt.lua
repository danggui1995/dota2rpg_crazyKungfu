...
function Phantasm( keys )
	...
	for i=1, images_count do
		...
		local origin = casterOrigin + (table.remove( vRandomSpawnPos, 1 ) or Vector(0,0,0))
		...
		for ability_remove=0,10 do
			local __temp_ability = illusion:GetAbilityByIndex(ability_remove)
			if __temp_ability then
				illusion:RemoveAbility(__temp_ability:GetAbilityName())
			end
		end
...
function RealityRift( keys )
	...
	local player 
	if caster:IsRealHero() then
		player = caster:GetPlayerOwnerID()
	end
	local ability = keys.ability
	local ability_level = ability:GetLevel() - 1
