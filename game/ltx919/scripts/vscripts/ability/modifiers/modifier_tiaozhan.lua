		...
		for k,v in pairs(GameRules.tiaozhan_t) do
			...
			if v["yx"] and v["yx"]==caster and (not v["jx"] or not v["jx"]:IsAlive()) then
				...
				elseif jl==3 then
					...
					local damage_ratio = hero.damage_ratio
					local equip_table = hero.equip
					...
					hero.damage_ratio = damage_ratio
					hero.equip = equip_table
					...
					GameRules.courier[id+1]:SetOwner(hero)
...
function modifier_tiaozhan_lua:OnDeath()
	if IsServer() then
		local caster = self:GetParent()
		for k,v in pairs(GameRules.tiaozhan_t) do
			if v["yx"] and v["jx"] and v["jx"]:IsAlive() and not v["yx"]:IsAlive() and v["yx"]==caster then
